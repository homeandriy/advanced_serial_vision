...
Advanced Serial Vision uses its own per-user application-data directory under the `homeandriy` organization, separate from the previous application. Startup attempts to write timestamped records to `startup.log` in that directory. If the OS denies access, startup continues without writing the log, retries once after the window appears, and shows the operator a modal message with the exact log folder and OS error. The Help menu can open that file when available. The fixed official GitHub repository is checked in a worker thread one minute after startup and on demand. A background check only informs the operator that an update is available. On Windows, the operator explicitly confirms installation from Help → Check for updates; the newer EXE is downloaded to a temporary folder and verified against its SHA-256 release digest while the application remains open. Only after successful verification does the application close, run the installer silently, and relaunch the updated executable. A per-user lock prevents a second application window from starting while an instance or updater is active. The installer uses the stable AppId and does not touch per-user SQLite data. On the next start, forward-only SQLite migrations are applied transactionally using PRAGMA user_version; migrations never delete or recreate the database. On Linux, the operator receives a manual-installation notice because a DEB update can require system privileges.
Packaged builds carry `VERSION` inside the bundle and never rely on their current working directory to show the installed version or check updates.
